...
class QuestionThree : Fragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        kotlin.runCatching {
            questionsViewModel.getMapForCurrentFragment(2)?.let {
        ...
        binding.composeView.apply {
            ...
            setContent {
                context?.let {
                    FragmentView(
                        3,
                        questionsViewModel.fetchQuestions(3, it),
                        questionsViewModel.screenOneContent(it),
                        showSelectedNext,
                        selectedItems,
                        onNextClick = {
                            logEvent("question_3_click_next", null)
                            kotlin.runCatching {
                                ...
                                logEvent("question_3_click_choose", Bundle().apply {
                                    putString("id_question_3", ids)
                                })
                                Log.i("TAG", "onViewCreated: $ids")

                                getParentActivity()?.navigate(
                                    QuestionThreeDirections.actionQuestionThreeToQuestionFour(),
                                    R.id.questionThree
                                )
                            }
                        },
                        onSkipClick = {
                            logEvent("question_3_click_skip", null)
                        ...
                        onItemClick = { index, isSelected ->
                            questionsViewModel.initializeStateForFragment(2,index, isSelected)
                            if (isSelected) {
