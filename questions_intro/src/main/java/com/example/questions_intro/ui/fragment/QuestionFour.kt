...
class QuestionFour : Fragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        kotlin.runCatching {
            questionsViewModel.getMapForCurrentFragment(3)?.let {
                selectedItems.putAll(it)
                showSelectedNext.value = selectedItems.isNotEmpty()
            }
        }
        logEvent("question_4_view", null)
    }
        ...
        binding.composeView.apply {
            ...
            setContent {
                context?.let {
                    FragmentView(
                        4,
                        questionsViewModel.fetchQuestions(4, it),
                        questionsViewModel.screenFourContent(it),
                        showSelectedNext,
                        selectedItems,
                        onNextClick = {
                            logEvent("question_4_click_next", null)
                            kotlin.runCatching {
                                ...
                                logEvent("question_4_click_choose", Bundle().apply {
                                    putString("id_question_4", ids)
                                })
                                Log.i("TAG", "onViewCreated: $ids")

                                getParentActivity()?.navigateToIntroActivity()
                            }
                        },
                        onSkipClick = {
                            logEvent("question_4_click_skip", null)
                        ...
                        onItemClick = { index, isSelected ->
                            questionsViewModel.initializeStateForFragment(3,index, isSelected)
                            if (isSelected) {
