...
class StickersPacksAdapter(
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (position < myList.size && position >= 0) {
            ...
            Glide.with(holder.packImg).load(if (obj.title == "offline") obj.file else obj.baseUrl + obj.file).sizeMultiplier(0.8f).placeholder(
                R.drawable.frame_place_holder_sticker
            ).into(holder.packImg)

//            holder.packImg.load(if (obj.title == "offline") obj.file else ApiConstants.BASE_URL_MEDIA + obj.file) {
//                placeholder(R.drawable.frame_place_holder_sticker)
//                crossfade(true)
//            }
